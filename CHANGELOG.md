...
**修复"为了不超框而把文字截断成 …"的问题：长度限制改为软性、严禁省略号截断。**

背景：之前把"不超 `max_chars`"写得偏硬（还建议 `--strict` 出框即拒绝），导致使用 Skill 的 AI 为凑数把文字砍断、结尾加 `...`，反而更难看。

变更：

- **`max_chars` 改为软性参考、非强制**（SKILL.md 铁律 #3、workflow.md A5/A6）：优先写自然完整精炼的文字，略微超出没关系（已留 20% 余量）。
- **严禁为凑长度而截断 + 加 `...` / `…` / `等等`**；太长就用更精炼措辞重写，或换更大版式，实在不行宁可轻微超出。
- **不再建议日常用 `--strict`**：出框检测仅作提示、不阻断保存（标准工作流命令已去掉 `--strict`）。
- `build_pptx.py`：出框检测降级为温和提示（不阻断、不再 `return` 失败码）；新增**结尾省略号截断的⚠️告警**，提醒改成完整表达。
- 铁律 #9：同级标题仍保持原字号，但措辞从"出框只缩短文字"改为"精炼重写、不截断不加省略号、不改字号"。

纯文档 + 脚本提示语气调整，无模板内容变动。升级：`python3 scripts/apply_update.py`。
